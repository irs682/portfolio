...
const BentoGrid = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 md:grid-rows-6 gap-6 h-full md:min-h-[900px]">
      ...
      <motion.div 
        whileHover={{ y: -8 }}
        className="md:col-span-8 md:row-span-3 glass-card rounded-[3rem] p-12 flex flex-col justify-between bg-slate-950 text-white overflow-hidden relative group"
      >
      ...
      <motion.div 
        whileHover={{ y: -8 }}
        className="md:col-span-4 md:row-span-4 glass-card rounded-[3rem] p-10 bg-white border-slate-100 shadow-2xl flex flex-col group"
      >
        <div className="flex items-center gap-4 mb-8">
          ...
          <h4 className="text-xl font-black text-slate-900">AI / ML Core</h4>
        </div>
          ...
          {skills.filter(s => s.category === "AI/ML").slice(0, 7).map((skill, i) => (
            <div key={i} className="space-y-2">
              <div className="flex justify-between items-end">
                <span className="text-xs font-black text-slate-900 uppercase tracking-widest">{skill.name}</span>
        ...
        <div className="mt-8 pt-8 border-t border-slate-100">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Advanced Focus</p>
          <p className="text-lg font-bold text-slate-900">RAG & LLM Workflows</p>
        </div>
      </motion.div>
      ...
      <motion.div 
        ...
        className="md:col-span-5 md:row-span-2 glass-card rounded-[3rem] p-10 bg-indigo-600 text-white border-none relative overflow-hidden"
      >
        ...
        <div className="relative z-10 h-full flex flex-col justify-between">
          <div>
            ...
            <p className="text-indigo-100 text-xs font-medium opacity-80">Web Scraping & Data Pipeline Engineering</p>
          </div>
      ...
      <motion.div 
        whileHover={{ y: -8 }}
        className="md:col-span-3 md:row-span-2 glass-card rounded-[3rem] p-8 bg-white border-slate-100 flex flex-col items-center justify-center text-center group"
      >
        <div className="w-16 h-16 bg-slate-950 rounded-[2rem] flex items-center justify-center mb-6 shadow-2xl group-hover:scale-110 transition-transform">
        ...
        <p className="text-xl font-black text-slate-900 leading-none">Ready for <br /> Innovation</p>
      </motion.div>

      {/* 5. Tools & Infrastructure (Bottom Block) */}
      <motion.div 
        whileHover={{ y: -8 }}
        className="md:col-span-12 md:row-span-1 glass-card rounded-[2.5rem] p-6 bg-slate-50 border-slate-200 flex items-center justify-between px-12"
      >
        <div className="flex items-center gap-6">
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Dev Tools</span>
          <div className="flex gap-8">
            {['Docker', 'Git', 'GitHub', 'VS Code', 'Postman'].map((tool) => (
              <span key={tool} className="text-xs font-bold text-slate-600 hover:text-primary-600 transition-colors cursor-default">{tool}</span>
            ))}
          </div>
        </div>
        <div className="hidden md:flex items-center gap-3">
          <Globe className="text-slate-400" size={18} />
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Deployments Enabled</span>
        </div>
      </motion.div>
